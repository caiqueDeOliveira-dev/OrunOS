  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-1" style={{ borderBottom: "1px solid var(--border)" }}>
        ...
        <div className="w-6 h-6 rounded-full flex items-center justify-center text-[8px] font-bold" style={{ background: "#8B5CF6", color: "#fff" }}>{avatarInitials}</div>
      </div>
          ...
          <>
            ...
            {(["pen", "rect", "circle", "text", "eraser"] as const).map((t) => (
              <button key={t} onClick={() => useWhiteboardStore.setState({ tool: t })}
                ...
                style={{ background: tool === t ? "rgba(192,0,24,0.15)" : "transparent", color: tool === t ? ACCENT : "var(--muted-foreground)" }}
                title={`${t.charAt(0).toUpperCase() + t.slice(1)} (${TOOL_SHORTCUTS[t]})`}>
                {TOOL_ICONS[t] || t}
                <span className="absolute -top-1.5 -right-1.5 text-[6px] px-0.5 rounded"
                  style={{ background: "var(--primary)", color: "var(--primary-foreground)", opacity: 0.8 }}>
                  {TOOL_SHORTCUTS[t]}
                </span>
              </button>
            ))}
            ...
            <div className="flex gap-1">
              {PALETTE.map((c) => (
                <button key={c} onClick={() => useWhiteboardStore.setState({ color: c })}
                  ...
                  style={{ background: c, border: color === c ? "2px solid var(--foreground)" : "2px solid transparent", transform: color === c ? "scale(1.15)" : "scale(1)" }} />
              ))}
            </div>
            <div className="w-px h-4" style={{ background: "var(--border)" }} />
            <div className="flex gap-1">
              {STROKE_WIDTHS.map((w) => (
                <button key={w} onClick={() => useWhiteboardStore.setState({ strokeWidth: w })}
                  ...
                  style={{ background: strokeWidth === w ? "rgba(192,0,24,0.15)" : "transparent" }}>
                  <div className="rounded-full" style={{ width: w * 2, height: w * 2, background: strokeWidth === w ? ACCENT : "var(--muted-foreground)" }} />
                </button>
              ))}
            </div>
            <div className="w-px h-4" style={{ background: "var(--border)" }} />
            <button onClick={undo} className="px-1.5 py-1 rounded text-[9px]" style={{ color: "var(--muted-foreground)" }} title="Desfazer (Ctrl+Z)">↩</button>
            <button onClick={redo} className="px-1.5 py-1 rounded text-[9px]" style={{ color: "var(--muted-foreground)" }} title="Refazer (Ctrl+Y)">↪</button>
            <div className="flex-1" />
            <span className="ws-small">{elements.length} elem</span>
            <button onClick={clearCanvas}
              className="ws-button text-[9px]" style={{ color: "#EF4444" }}>
              Limpar
            </button>
          </>
        )}
