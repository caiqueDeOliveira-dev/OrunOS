...
export function CenterPreview() {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 min-h-0">
      <div className="flex-1 flex items-center justify-center relative" style={{ background: "#050505", minHeight: 0 }}>
        <div data-preview className="relative flex items-center justify-center"
          ...
          <div className="absolute inset-0"
            style={{ background: activeClips.filter((c) => c.type === "video").length > 0 ? "linear-gradient(160deg, #141414 0%, #1C1C1C 40%, #141414 100%)" : "linear-gradient(160deg, #0A0A0C 0%, #101013 50%, #0A0A0C 100%)" }} />
          ...
          <div className="absolute top-2 left-2 flex items-center gap-2">
            <div className="px-1.5 py-0.5 rounded" style={{ background: "rgba(0,0,0,0.7)" }}>
              <span style={{ fontSize: 10, fontFamily: MONO, color: "#FFFFFF", letterSpacing: 1 }}>{formatTC(currentTimeFrame, fps)}</span>
            </div>
          </div>
          ...
          <div className="absolute top-2 right-2 flex items-center gap-2">
            ...
            <span style={{ fontSize: 9, fontFamily: MONO, color: "#5C5C5C" }}>/ {formatTCShort(totalFrames, fps)}</span>
            <div className="px-1.5 py-0.5 rounded" style={{ background: "rgba(195,0,47,0.85)" }}>
