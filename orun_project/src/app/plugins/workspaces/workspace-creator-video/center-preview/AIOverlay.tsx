import { MONO, ACCENT } from "../video-types";

// Overlay sits on the dark preview viewport — use fixed dark-surface colors,
// not theme tokens (which flip light in light mode and would vanish here).
  ...
  if (status === "generating") {
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2" style={{ background: "rgba(0,0,0,0.7)", zIndex: 50 }}>
        ...
        <div style={{ width: 120, height: 3, background: "#141414", borderRadius: 2, overflow: "hidden" }}>
          <div style={{ width: `${progress}%`, height: "100%", background: ACCENT, transition: "width 0.3s" }} />
        </div>
        <div style={{ fontSize: 7, fontFamily: MONO, color: "#8A8A92" }}>{progress}%</div>
      </div>
    );
  }
