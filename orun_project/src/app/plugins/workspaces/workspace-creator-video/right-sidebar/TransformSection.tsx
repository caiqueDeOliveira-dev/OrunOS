    ...
    <>
      ...
      <PropRow label={t("creator_video_scale")}>
        <div className="flex items-center gap-2">
          <input type="range" min={10} max={300} value={scale} onChange={(e) => useVideoStore.setState({ scale: Number(e.target.value) })} className="flex-1" style={{ accentColor: "#C00018", height: 3 }} />
          <span style={{ fontSize: 9, fontFamily: MONO, color: "#8B949E", minWidth: 28, textAlign: "right" }}>{scale}%</span>
        </div>
      </PropRow>

      <PropRow label={t("creator_video_rotation")}>
        <div className="flex items-center gap-2">
          <input type="range" min={-180} max={180} value={rotation} onChange={(e) => useVideoStore.setState({ rotation: Number(e.target.value) })} className="flex-1" style={{ accentColor: "#C00018", height: 3 }} />
          <span style={{ fontSize: 9, fontFamily: MONO, color: "#8B949E", minWidth: 28, textAlign: "right" }}>{rotation}&#176;</span>
        </div>
      </PropRow>
    </>
  );
