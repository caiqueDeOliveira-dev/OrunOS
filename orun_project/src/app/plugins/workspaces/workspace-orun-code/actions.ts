// plugins/workspaces/workspace-orun-code/actions.ts
// Orun Code — real file/terminal/security actions wired to the Electron
// `developer` bridge (window.orun.developer / /api/developer). This is the
// consolidation of the former Developer IDE actions into Orun Code.
...
const WORKSPACE_ID = "oruncode";

let registered = false;

let getStore: (() => any) | null = null;
export function setOCStoreGetter(getter: () => any) {
  getStore = getter;
}

function getOCState() {
  if (!getStore) throw new Error("Orun Code store not initialized");
  return getStore();
}

function notifyFileWritten() {
  try {
    window.dispatchEvent(new CustomEvent("oruncode:file-written"));
  } catch {}
}
...
const actions = {
  ...
  async create_file(params: Record<string, unknown>) {
    const filePath = String(params.path || "");
    if (!filePath) return { success: false, error: "path is required" };
    return actions.write_file({ path: filePath, content: String(params.content || "") });
  },
  ...
  async rename_file(params: Record<string, unknown>) {
    ...
    try {
      ...
      const language = getLanguage(newName);
  ...
  async execute_command(params: Record<string, unknown>) {
    ...
    try {
      ...
      store.setState((s: any) => ({
        terminalLines: [...s.terminalLines, { id: lineId, type: "input", text: `$ ${command}` }, { id: outputId, type: "error", text: e.message || `Failed to execute: ${command}` }],
      }));
...
export function registerOrunCodeActions() {
  if (registered) return;
  registered = true;
  registerWorkspaceActions(WORKSPACE_ID, actions);
}

export function unregisterOrunCodeActions() {
  if (!registered) return;
  registered = false;
...
export async function runOrunCodeAction(name: ActionName, params: Record<string, unknown> = {}): Promise<any> {
  const handler = actions[name];
  if (!handler) return { success: false, error: `Unknown action "${name}"` };
  try {
    return await handler(params);
  } catch (e: any) {
    return { success: false, error: e.message || `Action "${name}" failed` };
  }
}
