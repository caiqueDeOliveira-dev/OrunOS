...
export function ABTestsView() {
  ...
  return (
    <div className="p-4 space-y-4">
      <h3 className="text-[10px] tracking-wider uppercase" style={{ fontFamily: "'Sora', sans-serif", color: "var(--muted-foreground)" }} role="heading" aria-level={2}>
      ...
      {tests.map((test) => {
        ...
        return (
          <WorkspaceCard key={test.id}>
            <p className="text-[11px] font-medium mb-3" style={{ color: "var(--foreground)" }} role="heading" aria-level={3}>{test.name}</p>
              ...
              {(["A", "B"] as const).map((v) => {
                ...
                return (
                  <div key={v} className="p-2.5 rounded-lg" role="listitem" style={{
                    ...
                    <p className="text-center mt-2 text-[11px] font-medium" style={{ fontFamily: MONO, color: isWinner ? "#22C55E" : "var(--foreground)" }}>
                      {data.ctr}% CTR
                    </p>
                  </div>
                );
