        ...
        {[
          ...
          <WorkspaceCard key={i} padding={false}>
            <div className="p-2">
              ...
              <p className="text-[13px] font-medium" style={{ fontFamily: MONO, color: "var(--foreground)" }}>{s.value}</p>
            </div>
          </WorkspaceCard>
        ))}
      </div>
        ...
        return (
          <WorkspaceCard key={c.id}>
            <div className="flex items-start justify-between mb-2">
              <div>
                <p className="text-[11px] font-medium" style={{ color: "var(--foreground)" }} role="heading" aria-level={3}>{c.name}</p>
            ...
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
              <div className="h-full rounded-full transition-all" style={{ width: `${progress}%`, background: progress > 80 ? "#F59E0B" : "var(--accent, #C00018)" }} />
            </div>
            <div className="flex justify-between mt-1">
              <span className="text-[8px]" style={{ color: "var(--muted-foreground)" }}>{fmtCurrency(c.spent)}</span>
              <span className="text-[8px]" style={{ color: "var(--muted-foreground)" }}>{fmtCurrency(c.budget)}</span>
            </div>
