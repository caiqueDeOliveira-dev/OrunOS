...
export const AGENT_ENVIRONMENTS: Record<string, {
  ...
  Juridico: {
    ...
    secondary: "#D4AF37",
    bg: "linear-gradient(135deg, #EFF6FF 0%, #DBEAFE 50%, #BFDBFE 100%)",
    pattern: "radial-gradient(circle at 20% 80%, rgba(30,64,175,0.08) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(212,175,55,0.06) 0%, transparent 50%)",
    particles: ["⚖️", "📜", "🏛️", "📋", "🔍", "📑", "📸", "🎬"],
    ambientIcons: [Scale, BookOpen, FileText, Shield, Search, ClipboardList, Award, Users, Globe, Lock, Camera, Video, MessageSquare, Briefcase],
    description: "Escritorio Juridico Digital",
    tagline: "Seu advogado pessoal com catalogo de provas integrado",
    quickActions: [
      { label: "Receber Mídia WhatsApp", icon: MessageSquare, prompt: "Quero enviar fotos/videos como evidencia pelo WhatsApp" },
      { label: "Analisar Contrato", icon: FileText, prompt: "Analise este contrato e me aponte os pontos importantes" },
      { label: "Modelo de Documento", icon: ClipboardList, prompt: "Crie um modelo de documento juridico para mim" },
      { label: "Pesquisar Legislacao", icon: Search, prompt: "Pesquise a legislacao atualizada sobre este assunto" },
      { label: "Calculo Trabalhista", icon: Calculator, prompt: "Calcule os direitos trabalhistas com base nas informations" },
      { label: "Peticao Inicial", icon: FileText, prompt: "Ajude-me a redigir uma peticao inicial" },
    ],
    stats: [
      { label: "Evidências", value: "0", icon: Camera },
      { label: "Casos", value: "0", icon: Briefcase },
      { label: "Fotos", value: "0", icon: Image },
  ...
  AssistenteTecnico: {
    ...
    accent: "#D97706",
    secondary: "#B45309",
    bg: "linear-gradient(135deg, #FFFBEB 0%, #FEEBC8 50%, #FDE68A 100%)",
    pattern: "radial-gradient(circle at 20% 80%, rgba(217,119,6,0.08) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(180,83,9,0.06) 0%, transparent 50%)",
    particles: ["🔧", "⚡", "🔬", "🛠️", "📡", "💡"],
    ambientIcons: [Wrench, Cpu, Package, Settings, ShoppingCart, AlertTriangle, CheckCircle, ClipboardList, Clock, Box],
    description: "Oficina Profissional de Eletrônica",
    tagline: "Gestao completa de consertos, estoque de pecas e ferramentas profissionais",
    quickActions: [
      { label: "Diagnosticar Defeito", icon: AlertTriangle, prompt: "Meu aparelho esta com o seguinte problema: " },
      { label: "Registrar Conserto", icon: Plus, prompt: "Quero registrar um novo conserto na oficina" },
      { label: "Ver Estoque Baixo", icon: Package, prompt: "Liste as pecas com estoque baixo que preciso comprar" },
      { label: "Lista de Compras", icon: ShoppingCart, prompt: "Gere a lista de compras com pecas e ferramentas em falta" },
      { label: "Adicionar Peça", icon: Box, prompt: "Quero adicionar uma nova peca ao estoque" },
      { label: "Ferramentas", icon: Wrench, prompt: "Quero ver o estado das minhas ferramentas" },
    ],
    stats: [
      { label: "Consertos", value: "0", icon: Cpu },
      { label: "Peças", value: "0", icon: Package },
      { label: "Ferramentas", value: "0", icon: Wrench },
      { label: "Compras Pendentes", value: "0", icon: ShoppingCart },
    ],
  },
  Suporte: {
    icon: LifeBuoy,
    ...
    pattern: "radial-gradient(circle at 20% 80%, rgba(14,165,233,0.08) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(2,132,199,0.06) 0%, transparent 50%)",
    particles: ["🛟", "🔧", "📋", "🛡️", "⚙️", "📊"],
    ambientIcons: [LifeBuoy, Shield, Wrench, Search, ClipboardList, AlertTriangle, CheckCircle, FileText, Activity, Cpu],
    description: "Suporte Técnico do Sistema",
    tagline: "Suporte inteligente para erros, bugs e melhorias do sistema",
    quickActions: [
      { label: "Ver Erros", icon: FileText, prompt: "Mostre os erros recentes do sistema" },
      { label: "Diagnosticar Sistema", icon: Activity, prompt: "Faca um diagnostico completo do sistema" },
      { label: "Relatar Bug", icon: Bug, prompt: "Quero relatar um bug que encontrei no sistema" },
      { label: "Sugerir Melhoria", icon: Lightbulb, prompt: "Quero sugerir uma melhoria para o sistema" },
      { label: "Exportar Dados", icon: Database, prompt: "Exporte todos os dados de erros e sugestões salvos" },
      { label: "Central de Ajuda", icon: LifeBuoy, prompt: "Preciso de ajuda com um problema no sistema" },
    ],
    stats: [
      { label: "Erros", value: "0", icon: Bug },
      { label: "Registros", value: "0", icon: Database },
      { label: "Corrigidos", value: "0", icon: CheckCircle },
      { label: "Melhorias", value: "0", icon: Lightbulb },
    ],
  },
  "Personal Assistant": {
    icon: MessageSquare,
    ...
    pattern: "radial-gradient(circle at 20% 80%, rgba(14,165,233,0.08) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(2,132,199,0.06) 0%, transparent 50%)",
    particles: ["🤖", "💬", "📋", "🗓️", "🔔", "📝"],
    ambientIcons: [MessageSquare, Bell, Calendar, ClipboardList, Brain, Globe, Search, List, Clock, CheckCircle],
    description: "Assistente Pessoal Inteligente",
    tagline: "Seu assistente pessoal via WhatsApp — agende, lembre, pesquise e organize",
    quickActions: [
      { label: "Resumo Diario", icon: ClipboardList, prompt: "Faca um resumo do meu dia" },
      { label: "Criar Lembrete", icon: Bell, prompt: "Crie um lembrete para mim" },
      { label: "Pesquisar", icon: Search, prompt: "Pesquise na web sobre" },
      { label: "Agenda", icon: Calendar, prompt: "Mostre minha agenda de hoje" },
      { label: "Memoria", icon: Brain, prompt: "Salve esta informacao na memoria" },
      { label: "Lembrar", icon: Clock, prompt: "O que voou ja me disse sobre" },
    ],
    stats: [
      { label: "Tarefas", value: "0", icon: CheckCircle },
      { label: "Lembretes", value: "0", icon: Bell },
      { label: "Memorias", value: "0", icon: Brain },
      { label: "Grupos WA", value: "0", icon: MessageSquare },
    ],
  },
  "Home IA": {
    icon: House,
    ...
    pattern: "radial-gradient(circle at 20% 80%, rgba(22,163,74,0.08) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(14,165,233,0.06) 0%, transparent 50%)",
    particles: ["🏠", "💡", "🔒", "❄️", "📺", "🎥"],
    ambientIcons: [House, Lightbulb, Lock, Snowflake, Tv, Coffee, Cctv, Wifi, Bell, Zap],
    description: "Casa Inteligente",
    tagline: "Controle sua casa por voz — dispositivos, automacoes e cenas em um so lugar",
    quickActions: [
      { label: "Ligar Luzes", icon: Lightbulb, prompt: "Ligue as luzes da sala" },
      { label: "Cenas", icon: Zap, prompt: "Ative o modo cinema" },
      { label: "Automacoes", icon: Bell, prompt: "Execute a automacao boa noite" },
      { label: "Status da Casa", icon: House, prompt: "Qual o status da casa agora?" },
      { label: "Temperatura", icon: Thermometer, prompt: "Ajuste o ar-condicionado para 22 graus" },
      { label: "Portas", icon: Lock, prompt: "Tranque as portas e arme o alarme" },
    ],
    stats: [
      { label: "Dispositivos", value: "17", icon: Lightbulb },
      { label: "Luzes Acesas", value: "3", icon: Zap },
      { label: "Fechaduras", value: "3", icon: Lock },
      { label: "Automacoes", value: "4", icon: Bell },
    ],
  },
  "Cyber Security": {
    icon: Shield,
    accent: "#7C3AED",
    secondary: "#DC2626",
    bg: "linear-gradient(135deg, #0F172A 0%, #1E1B4B 50%, #2E1065 100%)",
    pattern: "radial-gradient(circle at 20% 80%, rgba(124,58,237,0.12) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(220,38,38,0.08) 0%, transparent 50%)",
    particles: ["🛡️", "🔐", "🔑", "🕵️", "⚠️", "💻"],
    ambientIcons: [Shield, KeyRound, Lock, AlertTriangle, MonitorCheck, Network, Server, Search, Bug, CheckCircle],
    description: "Auditoria de Seguranca",
    tagline: "Audite, proteja e blinde o seu sistema contra ameacas",
    quickActions: [
      { label: "Novo Scan", icon: RefreshCw, prompt: "Execute um novo scan de seguranca completo" },
      { label: "Relatorio", icon: Shield, prompt: "Mostre o ultimo relatorio de seguranca" },
      { label: "Credenciais", icon: KeyRound, prompt: "Verifique se ha credenciais expostas" },
      { label: "Portas", icon: Network, prompt: "Liste portas abertas no sistema" },
      { label: "Mitigar", icon: CheckCircle, prompt: "Sugira acoes para melhorar a seguranca" },
      { label: "Exportar", icon: Server, prompt: "Exporte o relatorio de seguranca" },
    ],
    stats: [
      { label: "Score", value: "100", icon: Shield },
      { label: "Criticos", value: "0", icon: AlertTriangle },
      { label: "Alto Risco", value: "0", icon: Lock },
      { label: "Mitigados", value: "0", icon: CheckCircle },
    ],
  },
  Hampton: {
    icon: Brain,
...
export const QUICK_ACTION_KEYS: Record<string, string[]> = {
  ...
  Juridico: ["analisar_contrato", "modelo_documento", "pesquisar_legislacao", "calculo_trabalhista", "peticao_inicial", "parecer_juridico"],
  AssistenteTecnico: ["diagnosticar_defeito", "esquema_eletronico", "calcular_componente", "guia_conserto", "identificar_peca", "aula_eletronica"],
  Suporte: ["ver_erros", "diagnosticar_sistema", "relatar_bug", "sugerir_melhoria", "exportar_dados", "central_ajuda"],
  "Personal Assistant": ["daily_summary", "create_reminder", "web_search", "agenda", "save_memory", "recall"],
  "Home IA": ["ligar_luzes", "cenas", "automacoes", "status_casa", "temperatura", "portas"],
  "Cyber Security": ["novo_scan", "relatorio", "credenciais", "portas", "mitigar", "exportar"],
};
...
export const STAT_KEYS: Record<string, string[]> = {
  ...
  Juridico: ["documentos", "contratos", "prazos", "pareceres"],
  AssistenteTecnico: ["consertos", "circuitos", "componentes", "aulas"],
  Suporte: ["erros", "registros", "corrigidos", "melhorias"],
  "Personal Assistant": ["tasks", "reminders", "memories", "whatsapp_groups"],
  "Home IA": ["dispositivos", "luzes_acesas", "fechaduras", "automacoes"],
  "Cyber Security": ["score", "criticos", "alto_risco", "mitigados"],
};
