    ...
    >
      ...
      >
        ...
        <p className="px-6 pt-4 text-[10px]" style={{ color: "#555" }}>
          Model + persona per agent, plus an optional daily schedule (e.g. Personal Trainer every morning).
          Only Nutritionist and Personal Trainer have real specialized behavior right now — the rest use their
          default persona until they get real logic too.
        </p>
            ...
            return (
              <div key={agent.name} className="py-2 border-b" style={{ borderColor: "#151515" }}>
                <div className="flex items-center gap-2.5">
                  ...
                  />
                  <button onClick={() => setEditingPrompt(editing ? null : agent.name)} title="Custom persona" style={{ color: editing ? "#FF1A2D" : "#555" }}><Pencil size={13} /></button>
                  <button
                ...
                {schedule.enabled && (
                  <div className="flex items-center gap-2 mt-1.5 ml-6 pl-3.5">
                    <span className="text-[9px]" style={{ color: "#555" }}>Send daily at</span>
                    <input
                ...
                {editing && (
                  <textarea
                    ...
                    placeholder={`Custom persona for ${agent.name} (leave blank to use the built-in default)`}
                    rows={2}
