  ...
  return (
    ...
    >
      ...
      >
        ...
        <label className="block text-[10px] tracking-wider uppercase mb-1.5" style={{ color: "var(--muted-foreground)" }}>n8n instance URL</label>
        <input
...
style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: "var(--muted-foreground)" }}
        ...
        {testState === "ok" && (
          <div className="mb-4 space-y-1">
            ...
            {workflows.slice(0, 6).map((w) => (
              <div key={w.id} className="flex items-center gap-1.5 text-[10px]" style={{ color: "var(--muted-foreground)" }}>
                <div className="w-1 h-1 rounded-full" style={{ background: w.active ? "#2ecc71" : "var(--muted-foreground)" }} />
                {w.name}
              </div>
            ))}
          </div>
        ...
        <label className="flex items-center justify-between mb-3 px-3 py-2.5 rounded-lg" style={{ background: "var(--secondary)", border: "1px solid var(--border)" }}>
        ...
        <p className="text-[10px] tracking-wider uppercase mb-2" style={{ color: "var(--muted-foreground)" }}>{t("automationSavedTitle")}</p>
        {actions.length === 0 && <p className="text-[10px] mb-2" style={{ color: "var(--muted-foreground)" }}>{t("automationNoneYet")}</p>}
        <div className="space-y-1.5 mb-3">
          {actions.map((a) => (
            <div key={a.name} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: "var(--secondary)", border: "1px solid var(--border)" }}>
              <div className="flex-1 min-w-0">
                <p className="text-xs truncate" style={{ fontFamily: "'Sora', sans-serif", color: "var(--foreground)" }}>{a.name}</p>
                {a.description && <p className="text-[10px] truncate" style={{ color: "var(--muted-foreground)" }}>{a.description}</p>}
              </div>
              <button onClick={() => removeAction(a.name)} style={{ color: "var(--muted-foreground)" }}><Trash2 size={13} /></button>
            </div>
          ))}
        </div>
        <div className="p-3 rounded-lg mb-4 space-y-2" style={{ background: "var(--secondary)", border: "1px dashed var(--border)" }}>
          <input value={newAction.name} onChange={(e) => setNewAction((p) => ({ ...p, name: e.target.value }))} placeholder={t("automationNamePlaceholder")} className="w-full px-2.5 py-1.5 rounded-md text-xs outline-none" style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: "var(--foreground)" }} />
          <input value={newAction.description} onChange={(e) => setNewAction((p) => ({ ...p, description: e.target.value }))} placeholder={t("automationDescPlaceholder")} className="w-full px-2.5 py-1.5 rounded-md text-xs outline-none" style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: "var(--foreground)" }} />
          <input value={newAction.webhookUrl} onChange={(e) => setNewAction((p) => ({ ...p, webhookUrl: e.target.value }))} placeholder="Webhook URL" className="w-full px-2.5 py-1.5 rounded-md text-xs outline-none" style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: "var(--foreground)" }} />
          <button onClick={addAction} className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs" style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: "var(--muted-foreground)" }}>
        ...
        {/* Manual webhook tester */}
        <p className="text-[10px] tracking-wider uppercase mb-2" style={{ color: "var(--muted-foreground)" }}>{t("automationTriggerNote")}</p>
        <input
        ...
        <div className="flex gap-2 mb-2">
          <input value={headerName} onChange={(e) => setHeaderName(e.target.value)} placeholder={t("automationHeaderName")} className="flex-1 px-3 py-2 rounded-lg text-xs outline-none" style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: "var(--muted-foreground)" }} />
          <input value={headerValue} onChange={(e) => setHeaderValue(e.target.value)} placeholder={t("automationHeaderValue")} className="flex-1 px-3 py-2 rounded-lg text-xs outline-none" style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: "var(--muted-foreground)" }} />
        </div>
        <textarea
        ...
        <button
          ...
          style={{ background: "#C00018", color: "var(--foreground)", opacity: !webhookUrl.trim() ? 0.4 : 1 }}
        >
          {webhookState === "sending" ? <Loader2 size={13} className="animate-spin" /> : <Send size={13} />}
          {t("automationSend")}
        </button>
        {webhookResult && (
          <pre className="mt-2 p-2 rounded-lg text-[9px] overflow-x-auto" style={{ background: "var(--secondary)", border: "1px solid var(--border)", color: webhookState === "error" ? "#C00018" : "var(--muted-foreground)", fontFamily: "'JetBrains Mono', monospace" }}>
            {webhookResult}
          </pre>
        )}
      </motion.div>
