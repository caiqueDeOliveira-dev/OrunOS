      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "#1a1a1a" }}>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm tracking-widest uppercase" style={{ fontFamily: "'Sora', sans-serif", color: "#F5F5F5" }}>Usage Today</span>
          </div>
          <button onClick={onClose} style={{ color: "#666" }}><X size={16} /></button>
        </div>

        <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
          {!isElectron && <p className="text-[11px]" style={{ color: "#555" }}>Usage tracking only works inside the packaged Electron app.</p>}
          {isElectron && loading && <p className="text-[11px]" style={{ color: "#555" }}>Loading…</p>}
          {isElectron && !loading && rows.length === 0 && (
            <p className="text-[11px]" style={{ color: "#555" }}>No requests yet today. Send a message to Hampton and check back here.</p>
          )}
          {rows.length > 0 && (
            <div className="space-y-3">
              ...
              <p className="text-[10px] pt-1" style={{ color: "#444" }}>
                {totalRequests} total request{totalRequests === 1 ? "" : "s"} across all providers today.
                Token counts come from each provider's own reporting — some (like Ollama) only report after a response finishes.
              </p>
            </div>
          )}
          ...
          {ttsRows.length > 0 && (
            <div className="mt-5 pt-4 border-t space-y-3" style={{ borderColor: "#1a1a1a" }}>
              <p className="text-[10px] tracking-wider uppercase" style={{ color: "#555" }}>Voice (TTS)</p>
              {ttsRows.map((r) => (
