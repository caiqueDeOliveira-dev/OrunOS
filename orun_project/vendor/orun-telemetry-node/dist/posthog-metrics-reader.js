...
exports.PostHogMetricsReader = void 0;
/**
 ...
 */
class PostHogMetricsReader {
    config;
    constructor(config) {
        this.config = config;
    }
    async runQuery(hogql) {
        const response = await fetch(`${this.config.host}/api/projects/${this.config.projectId}/query/`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            throw new Error(`[@orun/telemetry] Falha na query PostHog: ${response.status}`);
        }
    ...
    `);
        return rows.map((row) => ({
            ...
            actionRegistrationRate: row.actionsExpected > 0
...
exports.PostHogMetricsReader = PostHogMetricsReader;
