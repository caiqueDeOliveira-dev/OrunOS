...
export class MusixmatchLyricsProvider implements ILyricsProvider {
  constructor(private readonly apiKey: () => Promise<string>) {} // ← plug in last, via ISecretStore
  ...
  async getSyncedLyrics(track: Pick<Track, 'title' | 'artist' | 'durationMs'>): Promise<SyncedLyrics | null> {
    ...
    const subtitleRes = await fetch(
      `${API_BASE}/track.subtitle.get?track_id=${trackId}&subtitle_format=lrc&apikey=${key}`,
    );
    if (!subtitleRes.ok) return null;
    const subtitleData = await subtitleRes.json();
    const lrc: string | undefined = subtitleData?.message?.body?.subtitle?.subtitle_body;
    if (!lrc) return null;
