...
import type { PlaybackState, Playlist, SyncedLyrics, Track } from '@orun/music-core/schemas';

export function useSpotifyPlayer(player: ISpotifyPlayer) {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      setConnected(ok);
      if (ok) setState(await player.getState());
      return ok;
    } finally {
      setConnecting(false);
    }
...
export function usePlaylists(store: IPlaylistStore, sync: IPlaylistSync) {
  ...
  const create = useCallback(async (title: string, description = '') => {
    const now = Date.now();
  ...
  const exportToSpotify = useCallback(async (playlistId: string, tracks: Track[]) => {
    const playlist = await store.get(playlistId);
    if (!playlist) return;
  ...
  const importFromSpotify = useCallback(async () => {
    ...
    for (const p of imported) await store.upsert(p);
    await refresh();
...
export function useSyncedLyrics(provider: ILyricsProvider, track: Track | null, positionMs: number) {
  const [lyrics, setLyrics] = useState<SyncedLyrics | null>(null);
  const cache = useRef(new Map<string, SyncedLyrics | null>());
  ...
  useEffect(() => {
    ...
    if (cached !== undefined) { setLyrics(cached); return; }
