...
class AuthClient {
    constructor(config) {
        ...
        this.auditLogger = new AuditLogger_1.AuditLogger(config.supabase);
    }
    getState() {
        return this.state;
    }
    ...
    async initialize() {
        ...
        const storedRefreshToken = await tokenStore.getItem(ISecureTokenStore_1.TOKEN_STORE_KEYS.REFRESH_TOKEN);
        if (!storedRefreshToken) {
            this.setState({ status: 'unauthenticated' });
            return;
        }
        const { data, error } = await supabase.auth.setSession({
            access_token: (await tokenStore.getItem(ISecureTokenStore_1.TOKEN_STORE_KEYS.ACCESS_TOKEN)) ?? '',
        ...
        supabase.auth.onAuthStateChange(async (event, session) => {
            if (event === 'TOKEN_REFRESHED' && session) {
                await tokenStore.setItem(ISecureTokenStore_1.TOKEN_STORE_KEYS.ACCESS_TOKEN, session.access_token);
                await tokenStore.setItem(ISecureTokenStore_1.TOKEN_STORE_KEYS.REFRESH_TOKEN, session.refresh_token);
    ...
    async verifyPassword(password) {
        ...
        if (!email)
            throw new Error('[@orun/identity] verifyPassword chamado sem usuário autenticado.');
    ...
    async registerPasskey() {
        ...
        if (!auth.registerPasskey) {
            throw new Error('[@orun/identity] registerPasskey indisponível — confirme @supabase/supabase-js >= 2.105.0 e Passkeys habilitado no dashboard.');
        }
    ...
    async signInWithPasskey() {
        ...
        if (!auth.signInWithPasskey) {
            throw new Error('[@orun/identity] signInWithPasskey indisponível — confirme @supabase/supabase-js >= 2.105.0 e Passkeys habilitado no dashboard.');
        }
        const { data, error } = await auth.signInWithPasskey();
        if (error)
            throw error;
        if (!data?.session)
            throw new Error('[@orun/identity] signInWithPasskey não retornou sessão.');
        await this.persistAndHydrate(data.session.access_token, data.session.refresh_token, data.session.user.id);
    ...
    async persistAndHydrate(accessToken, refreshToken, userId) {
        const { tokenStore } = this.config;
        await tokenStore.setItem(ISecureTokenStore_1.TOKEN_STORE_KEYS.ACCESS_TOKEN, accessToken);
        await tokenStore.setItem(ISecureTokenStore_1.TOKEN_STORE_KEYS.REFRESH_TOKEN, refreshToken);
        await this.hydrateFromSession(accessToken, refreshToken, userId);
    }
    async hydrateFromSession(accessToken, _refreshToken, userId) {
