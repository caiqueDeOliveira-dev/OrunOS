...
contextBridge.exposeInMainWorld("orun", {
  ...
  calcom: {
    health: () => ipcRenderer.invoke("calcom:health"),
    listEventTypes: () => ipcRenderer.invoke("calcom:list-event-types"),
    getAvailability: (opts) => ipcRenderer.invoke("calcom:get-availability", opts),
    findSlots: (opts) => ipcRenderer.invoke("calcom:find-slots", opts),
    createBooking: (input) => ipcRenderer.invoke("calcom:create-booking", input),
    cancelBooking: (input) => ipcRenderer.invoke("calcom:cancel-booking", input),
    reserveSlot: (input) => ipcRenderer.invoke("calcom:reserve-slot", input),
  },
  app: {
  ...
  vpn: {
    ...
    addServer: (config) => ipcRenderer.invoke("vpn:add-server", config),
    removeServer: (id) => ipcRenderer.invoke("vpn:remove-server", id),
    getPeers: (serverId) => ipcRenderer.invoke("vpn:get-peers", serverId),
    connect: (serverId, peerId) => ipcRenderer.invoke("vpn:connect", serverId, peerId),
    disconnect: (serverId) => ipcRenderer.invoke("vpn:disconnect", serverId),
    getState: (serverId) => ipcRenderer.invoke("vpn:get-state", serverId),
    setKillSwitch: (serverId, enabled) => ipcRenderer.invoke("vpn:set-kill-switch", serverId, enabled),
    provisionPeer: (serverId, name) => ipcRenderer.invoke("vpn:provision-peer", serverId, name),
  },
  system: {
